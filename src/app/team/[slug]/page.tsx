...
export default function ProfilePage() {
  ...
  // Get prev/next profiles for navigation
  const currentIndex = teamProfiles.findIndex((p) => p.slug === slug);
  const prevProfile = currentIndex > 0 ? teamProfiles[currentIndex - 1] : null;
  const nextProfile = currentIndex < teamProfiles.length - 1 ? teamProfiles[currentIndex + 1] : null;
  ...
  return (
    <main className="min-h-screen" style={{ background: "#FFFFFF" }}>
      <section className="relative lg:min-h-screen grid grid-cols-1 lg:grid-cols-[420px_1fr] xl:grid-cols-[480px_1fr]">
        ...
        <div className="relative border-r px-8 lg:px-12 pt-44 lg:pt-52 pb-6 lg:pb-16 flex flex-col" style={{ background: "#FFFFFF", borderColor: "rgba(0,0,0,0.06)", minHeight: "auto" }}>
          ...
          >
            ...
            {profile.profileNumber && ` · ${profile.profileNumber}`}
          </motion.p>
          ...
          {/* Navigation buttons */}
          {/* Navigation buttons — hidden on mobile, shown in left panel on desktop */}
          <div className="mt-auto pt-10 hidden lg:flex items-center gap-4">
            {prevProfile ? (
              <Link
                href={`/team/${prevProfile.slug}`}
      ...
      {/* Mobile navigation — below profile content */}
